...
export default function Team() {
  return (
    <section id="team" className="py-24 bg-zinc-950 text-white relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 max-w-6xl mx-auto">
          {teamMembers.map((member, index) => (
            <div
              key={index}
              className="bg-zinc-900/80 border border-zinc-800/80 p-8 rounded-3xl shadow-xl flex flex-col justify-between hover:border-zinc-700 transition-all duration-300"
            >
              <div>
                {/* Header Profile */}
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 mb-6 text-center sm:text-left">
                  <div className="relative w-28 h-28 rounded-2xl overflow-hidden border-2 border-blue-500/40 shrink-0 bg-zinc-800">
                    <Image
                      src={member.image}
                      alt={member.name}
                      fill
                  ...
                  <div>
                    ...
                    <p className="text-blue-400 font-medium text-sm mb-1">{member.role}</p>
                    <p className="text-zinc-400 text-sm flex items-center justify-center sm:justify-start gap-1">
                      🎓 {member.university}
                    </p>
                  </div>
                </div>
                    ...
                    <ul className="space-y-1">
                      {member.education.map((edu, idx) => (
                        <li key={idx} className="text-sm text-zinc-300 flex items-start gap-2">
                          <span className="text-blue-500">•</span> {edu}
                        </li>
                      ))}
                    </ul>
                  </div>
                    ...
                    <div className="space-y-4">
                      {Array.isArray(member.experience) ? (
                        member.experience.map((exp, idx) => (
                          <div key={idx} className="bg-zinc-950/50 p-4 rounded-xl border border-zinc-800/60">
                            <h5 className="text-sm font-bold text-blue-400">{exp.title}</h5>
                            <p className="text-xs font-medium text-zinc-400 mb-2">{exp.company}</p>
                            <ul className="space-y-1">
                              {exp.details.map((detail, dIdx) => (
                                <li key={dIdx} className="text-xs text-zinc-300 flex items-start gap-2">
                                  <span className="text-blue-500 mt-0.5">▪</span> {detail}
                                </li>
                              ))}
                            </ul>
                          </div>
                        ...
                        <p className="text-sm text-zinc-300">💼 {member.experience}</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {member.skills.map((skill, idx) => (
                        <span key={idx} className="px-3 py-1 bg-zinc-800 text-blue-300 text-xs font-medium rounded-lg border border-zinc-700">
                          {skill}
                        </span>
                      ))}
                    </div>
